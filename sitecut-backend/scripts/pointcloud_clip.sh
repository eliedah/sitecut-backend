...
# Direct PDAL pipeline against an EPT/COPC SOURCE (set SOURCE=https://.../ept.json or .copc.laz)
if [ -z "$SOURCE" ]; then
...
{
  "pipeline": [
    { "type":"readers.ept", "filename":"${SOURCE}",
      "bounds":"([${W},${E}],[${S},${N}])", "spatialreference":"EPSG:4326" },
    { "type":"filters.sample", "radius":0.0 },
    { "type":"filters.decimation", "limit": ${MAXPTS} },
