...
import { getPointCloud, getPointCloudLAZ, lidarSourcesFor } from './adapters/pointcloud.js';
import { createReadStream } from 'node:fs';

const app = Fastify({ logger: true });
const _origins = (process.env.CORS_ORIGIN || '*').split(',').map(s => s.trim().replace(/\/+$/, '')).filter(Boolean);
await app.register(cors, {
  origin: _origins.includes('*') ? true : (origin, cb) => {
    if (!origin) return cb(null, true);                    // same-origin / curl
    cb(null, _origins.includes(origin.replace(/\/+$/, ''))); // ignore trailing slash
  }
});
...
// Which free LiDAR sources cover this point
app.get('/api/lidar-sources', async (req, reply) => {
  const { lat, lon } = req.query;
  if (lat === undefined || lon === undefined) return reply.code(400).send({ error: 'lat,lon required' });
  return { sources: lidarSourcesFor(+lat, +lon) };
});
